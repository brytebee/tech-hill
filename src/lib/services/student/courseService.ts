...
export class StudentCourseService {
    ...
    try {
      ...
      const moduleProgress = course.modules.map((module: any) => {
        ...
        const completedTopics = topics.filter(
          (t: any) => progressMap.get(t.id) === "COMPLETED",
        ).length;

        const requiredTopics = topics.filter((t: any) => t.isRequired).length;
        const completedRequiredTopics = topics.filter(
          (t: any) => t.isRequired && progressMap.get(t.id) === "COMPLETED",
        ).length;
  ...
  static async getTopicAccessibility(userId: string, topicId: string) {
    try {
      ...
      // 1. Check if topic is a "Free Preview"
      if (topic.isPreview) {
        return { canAccess: true, isPreview: true };
      }
  ...
  static async getModuleProgress(userId: string, moduleId: string) {
    try {
      ...
      const requiredTopics = module.topics.filter(
        (t: any) => t.isRequired,
      ).length;
      const completedRequiredTopics = module.topics.filter(
        (t: any) =>
          t.isRequired &&
          topicProgress.some(
            (p: any) => p.topicId === t.id && p.status === "COMPLETED",
          ),
      ).length;
    ...
    try {
      ...
      for (const question of quiz.questions) {
        ...
        if (
          ...
          isCorrect =
            ...
            correctOptions.every((id: any) => userAnswers.includes(id));
          pointsEarned = isCorrect
            ? question.points
            : Math.round(
                question.points *
                  (userAnswers.filter((id: any) => correctOptions.includes(id))
                    .length /
      ...
      const attempt = await prisma.quizAttempt.create({
        data: {
          ...
          answers: {
            create: questionResults.map((result: any) => ({
              questionId: result.questionId,
              isCorrect: result.isCorrect,
              points: result.pointsEarned,
              // Add other fields if needed, or mapping
            })),
          },
          completedAt: new Date(),
        },
  ...
  static async createCourseCertificate(userId: string, courseId: string) {
    try {
      ...
      // 2. Fetch course details including commerce config
      ...
      // 3. Payment Validation for paid certificates
      if (
        !course.isCertificateFree &&
        Number(course.certificatePrice || 0) > 0
      ) {
        const hasPaidForCertificate = await prisma.transaction.findFirst({
          where: {
            userId,
            courseId,
            status: "SUCCESS",
        ...
        if (!hasPaidForCertificate) {
          // Instead of throwing, we might just return null or a specific status
          // The UI will handle showing a "Buy Certificate" button if it's missing
          return null;
        }
      }
      ...
      const certificate = await prisma.certificate.create({
        data: {
          ...
          courseId,
          title: course.title,
          finalScore: 100, // Placeholder or calculate
          issuedAt: new Date(),
          validFrom: new Date(),
        },
      });
