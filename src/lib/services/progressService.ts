...
export class ProgressService {
  ...
  static async updateTopicProgress(
    ...
    if (isProject && completed) {
      // For practice projects, a submission must be APPROVED
      const isApproved = topic.submissions.some(s => (s.status as string) === "APPROVED");
      if (!isApproved) {
        canComplete = false;
      }
    }
    ...
    } else if (completed && !canComplete && (hasAssessments || isProject)) {
      // User tried to complete but hasn't passed all assessments/evaluations
  ...
  static async updateModuleProgress(userId: string, moduleId: string) {
    ...
    const passedModule = (() => {
      ...
      if (hasAssessmentTopics && averageScore) {
  ...
  static async updateCourseProgress(userId: string, courseId: string) {
    ...
    if (allModulesCompleted && passedCourse) {
      try {
        await CertificateService.issueCertificate(
          userId,
          courseId,
          finalGrade,
          false // Currently auto-issuing. For HITL, make this configurable.
        );
      } catch (err) {
      ...
      // and propagate the completion up to the track layer.
      try {
        ...
        for (const enrollment of trackEnrollments) {
          const isPartOfTrack = enrollment.track.courses.some(
            (tc: any) => tc.courseId === courseId
          );
          if (isPartOfTrack) {
            await TrackService.updateTrackProgress(
              userId,
              enrollment.trackId,
              courseId
            );
        ...
        console.error("Failed to update track progress after course completion:", err);
      }
    }
