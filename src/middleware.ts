...
export default withAuth(
  function middleware(req) {
    ...
    // Preserve query params (e.g. ?payment=success) so the target page can read them.
    if (pathname === "/dashboard" && isAuth) {
      const role = token.role?.toLowerCase();
      const targetUrl = new URL(`/${role}`, req.url);
...
export const config = {
  matcher: ["/((?!api/auth|_next/static|_next/image|favicon.ico|public|images|courses|projects).*)"],
};
