...
export async function POST(
  ...
  try {
    ...
    if (
      requiresOptions.includes(validatedData.questionType) &&
    ...
    if (
      validatedData.questionType === "TRUE_FALSE" &&
    ...
    if (
      ...
      const hasCorrectAnswer = validatedData.options?.some(
    ...
    if (
      validatedData.questionType === "MULTIPLE_CHOICE" ||
      validatedData.questionType === "TRUE_FALSE"
    ) {
      const correctAnswers =
        validatedData.options?.filter((option) => option.isCorrect) || [];
    ...
    logger.error("quizzes:quizId:questions", "POST /api/quizzes/[quizId]/questions error:", error);

    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: "Invalid data", details: (error as any).issues },
        { status: 400 }
      );
    }
...
export async function GET(
  ...
  try {
    ...
    logger.error("quizzes:quizId:questions", "GET /api/quizzes/[quizId]/questions error:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
