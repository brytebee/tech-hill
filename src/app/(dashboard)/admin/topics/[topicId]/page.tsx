...
export default async function TopicDetailsPage({
    ...
    topic.quizzes?.reduce(
      (total: any, quiz: any) =>
        ...
        quiz.questions.reduce(
          (quizTotal: any, q: any) => quizTotal + q.points,
          0
        ),
      0
    ) || 0;
  ...
  const totalQuestions =
    topic.quizzes?.reduce(
      (total: any, quiz: any) => total + quiz.questions.length,
      0
    ) || 0;
    ...
    >
      <div className="space-y-8">
        ...
        <div className="flex justify-between items-start">
          ...
          <TopicActions
            topic={topic as any}
            totalTopics={topic.module._count?.topics || 0}
            canMoveUp={topic.orderIndex > 1}
            canMoveDown={topic.orderIndex < (topic.module._count?.topics || 0)}
          />
        </div>
            ...
            <CardContent className="space-y-4">
              ...
              {topic.attachments && topic.attachments.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1">
                    {topic.attachments.map((attachment: any, index: any) => (
                      <a
                        key={index}
                        href={attachment}
                        target="_blank"
              ...
              {topic.dependentTopics.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1">
                    {topic.dependentTopics.map((depTopic: any) => (
                      <Link
                        key={depTopic.id}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 mt-3 ml-6 text-sm text-gray-500">
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            <span>
                              {quiz.questions.reduce(
                                (total: any, q: any) => total + q.points,
                                0
                              )}{" "}
                              points
                            </span>
