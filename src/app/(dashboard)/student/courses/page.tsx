...
async function getCoursesData(userId: string, searchParams: PageProps['searchParams']) {
  try {
...
function SearchAndFilters({ searchParams }: { searchParams: PageProps['searchParams'] }) {
  return (
    <Card>
      <CardContent className="pt-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <Select defaultValue={searchParams.difficulty || ''} name="difficulty">
...
export default async function StudentCoursesPage({ searchParams }: PageProps) {
  ...
  const { courses, totalPages, totalCourses, currentPage, enrollments } = 
    await getCoursesData(session.user.id, searchParams);
