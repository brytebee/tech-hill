"use client";
// components/shared/ResetTrackButton.tsx
...
import { RotateCcw, Loader2, AlertTriangle, Layers } from "lucide-react";

interface ResetTrackButtonProps {
  /** The TrackEnrollment.id to reset. */
...
export function ResetTrackButton({
  ...
  const handleReset = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/track-enrollments/${trackEnrollmentId}/reset`, {
        method: "POST",
      });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button {...triggerProps}>
          ...
          {variant === "admin" ? "Force Reset Path" : "Restart Path"}
        </Button>
      </DialogTrigger>
      ...
      <DialogContent className="sm:max-w-md rounded-3xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <DialogHeader className="space-y-4">
          ...
          <DialogDescription className="text-center text-slate-500 font-medium leading-relaxed">
            This will permanently wipe ALL progress across all{" "}
            ...
            , including quiz attempts and module completions.
            <br />
            <span className="text-rose-600 dark:text-rose-400 font-black">
              The entire path restarts from Course 1. This cannot be undone.
            </span>
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="flex-col sm:flex-row gap-3">
          ...
          <Button
            onClick={handleReset}
            disabled={loading}
            className="flex-1 h-11 rounded-xl font-black uppercase tracking-widest text-xs bg-rose-600 hover:bg-rose-500 text-white shadow-lg shadow-rose-500/20"
          >
            {loading ? (
            ...
            {loading ? "Resetting…" : "Yes, Reset Path"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
