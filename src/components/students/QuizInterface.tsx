...
export function QuizInterface({ quiz, attempts, userId, topicId }: QuizInterfaceProps) {
  ...
  const renderQuestion = (question: QuizQuestion, index: number) => {
    ...
    switch (question.questionType) {
                ...
                <Checkbox
                  ...
                  onCheckedChange={(checked) => {
                    if (checked) {
                      ...
                      handleAnswerChange(question.id, selectedOptions.filter(id => id !== option.id));
                    }
                  }}
                />
                <Label htmlFor={option.id} className="flex-1 cursor-pointer">
      ...
      case 'ESSAY':
        const wordCount = ((answer as string || '').match(/\S+/g) || []).length;
        return (
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm text-gray-500">
              ...
              {question.minWords && (
                <span className={wordCount >= question.minWords ? 'text-green-600' : 'text-red-600'}>
